...
export default function Search() {
  ...
  useEffect(() => {
    ...
    if (user && stockData) {
      fetch(`${import.meta.env.VITE_API_URL}/get_watchlist/${user.uid}?type=stocks`)
        .then(res => res.json())
        .then(data => {
  ...
  useEffect(() => {
    ...
    if (user && stockData && optionsState.showOptions) {
      fetch(`${import.meta.env.VITE_API_URL}/get_watchlist/${user.uid}?type=options`)
        .then(res => res.json())
        ...
        try {
          ...
          if (showOptionsParam === "true") {
            ...
            const response = await fetch(
              `${import.meta.env.VITE_API_URL}/options/${symbolFromUrl.toUpperCase()}?expiration=${expirationParam || ''}`
            );
  ...
  const fetchStockData = async (symbolToFetch = null) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/stocks/${searchSymbol.toUpperCase()}`);
      if (!response.ok) {
