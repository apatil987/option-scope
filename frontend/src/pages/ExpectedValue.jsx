...
const ExpectedValue = () => {
  ...
  const handleOptionSelect = async (option) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/option_details/${option.symbol}/${option.expiration}/${option.strike}/${option.option_type}`
      );
  ...
  const calculateEV = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/calculate_ev`, {
        method: "POST",
